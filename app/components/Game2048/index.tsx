...
const Game2048 = () => {
  ...
  const tileColors: Record<number, string> = {
    0: 'bg-zinc-400',
    2: 'bg-stone-200 text-zinc-800',
    4: 'bg-stone-300 text-zinc-800',
  ...
  const NewMoveBox = (direction: string, grid: number[][]): number[][] => {
    switch (direction) {
      ...
      case 'bottom': {
        const t = transpose(grid);
        const moved = t.map((row) => mergeLine([...row].reverse()).reverse());
        return transpose(moved);
      }
  ...
  return (
    <div className="relative flex flex-1">
      ...
      <div
        className="m-auto grid h-full min-h-[250px] w-full min-w-[250px] grid-cols-4 gap-3"
        style={{
          touchAction: 'none',
        ...
        {grid.map((row, r) =>
          row.map((value, c) => (
            <div
              ...
              key={`${r}-${c}`}
              className={`flex aspect-square min-h-[20px] min-w-[20px] items-center justify-center rounded-lg text-2xl font-bold ${tileColors[value] ?? 'bg-black text-white'}`}
            >
              {value !== 0 && value}
            </div>
          ))
      ...
      {gameOver && (
        <div className="pointer-events-auto absolute inset-0 z-10 flex flex-col items-center justify-center">
          ...
          >
            <p className="mb-6 text-center text-5xl font-extrabold text-white">GAME OVER</p>
            <button
